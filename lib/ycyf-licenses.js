/* ============================================================
   You Call Yourself A Friend license store
   ------------------------------------------------------------
   Backed by Vercel KV. Mirrors lib/lq-licenses.js exactly, but
   under a SEPARATE key namespace (ycyf:*) so a buyer who owns both
   Lovers Quest and YCYF with the same email never collides.
     ...
     ycyf:email:<lowercased>:license  -> token   (reverse lookup)

   Licensing model (per Dennis, #82):
     - A YCYF license authorizes the buyer's device(s) to CREATE
       rooms (host) indefinitely. Joiners never need a license.
     - Hard cap of 2 active devices (mirrors LQ: phone + tablet).
       Activating a 3rd evicts the oldest by registeredAt.
     - A "device" is a short stable random id the client computes
       once and stores in localStorage.
   ============================================================ */
...
async function createLicense({ email, firstName, product, stripeSessionId }) {
  ...
  const record = {
    ...
    product: product || 'ycyf',
    devices: [],
    createdAt: new Date().toISOString(),
    stripeSessionId: stripeSessionId || null
  };
  await kv.set(`ycyf:license:${token}`, record);
  await kv.set(`ycyf:email:${record.email}:license`, token);
  return { token, record };
}

async function getLicense(token) {
  if (!token) return null;
  return await kv.get(`ycyf:license:${token}`);
}

async function findLicenseByEmail(email) {
  const e = normalizeEmail(email);
  if (!e) return null;
  const token = await kv.get(`ycyf:email:${e}:license`);
  if (!token) return null;
  const record = await kv.get(`ycyf:license:${token}`);
...
/*
   ...
   Returns: { ok, license, evictedDevice? }
     ...
     - If at cap: evict the oldest device by registeredAt, add the new one.
*/
...
/*
   Check whether a device's fingerprint is currently authorized on this
   license. Used on load to confirm the local token is still valid (e.g.
   another device may have bumped this one past the 2-device cap).
*/
