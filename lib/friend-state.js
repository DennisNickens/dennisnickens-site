...
// while everyone is still guessing, other players' guesses before reveal).
...
function publicView(room, viewerId) {
  ...
  if (out.phase === 'playing' && out.subPhase === 'guessing') {
    // Hide truth from non-Subject viewers
    if (viewerId !== out.currentSubjectId) {
      out.subjectAnswer = null;
    }
    // Hide other players' guesses; keep the viewer's own
    var allGuesses = out.guesses || {};
    var visible = {};
    if (viewerId && allGuesses[viewerId]) visible[viewerId] = allGuesses[viewerId];
    out.guesses = visible;
    // Expose only the COUNT of guesses submitted so the UI can show progress
    out.guessersSubmitted = Object.keys(room.guesses || {}).length;
  }
  return out;
}
...
async function startGame({ code, hostId }) {
  ...
  room.round = 1;
  room.turn = 0;
  room.turnOrder = shuffle(allPlayerIds);
  room.currentSubjectId = room.turnOrder[0];
...
// Scoring engine. Returns a detail object describing the reveal:
//   { cardId, type, truth, guesses, correctGuessers, pairDeltas }
// pairDeltas is { pairId: pointsToAddBeforeCap }.
function scoreCard(card, guesses, truth, subjectId, pairs) {
  var detail = {
    cardId: card.id,
    type: card.type,
    truth: truth,
  ...
  var correctGuessers = [];
  for (var pid in guesses) {
    if (Object.prototype.hasOwnProperty.call(guesses, pid) && guesses[pid] === truth) {
      correctGuessers.push(pid);
    }
  }
  detail.correctGuessers = correctGuessers;

  var guesserPoints = (card.scoring && card.scoring.guesserPoints) || 0;
  var subjectPoints = (card.scoring && card.scoring.subjectPoints) || 0;

  // Each correct guesser adds guesserPoints to their pair's score
  correctGuessers.forEach(function (pid) {
    var pr = findPairByPlayerId(pid, pairs);
    if (!pr) return;
    detail.pairDeltas[pr.id] = (detail.pairDeltas[pr.id] || 0) + guesserPoints;
  });

  // Subject's pair bonus
  var subjPair = findPairByPlayerId(subjectId, pairs);
  if (subjPair) {
    var bonus = 0;
    if (card.type === 'group_vote') {
      // group_vote: subject earns the flat bonus only if MAJORITY of
      // guessers pointed correctly.
      var totalGuessers = Object.keys(guesses || {}).length;
      if (totalGuessers > 0 && correctGuessers.length > totalGuessers / 2) {
        bonus = subjectPoints;
      }
    } else {
      // mc4/mc6/tf: subject earns subjectPoints per correct guesser.
      bonus = correctGuessers.length * subjectPoints;
    }
    if (bonus > 0) {
      detail.pairDeltas[subjPair.id] = (detail.pairDeltas[subjPair.id] || 0) + bonus;
    }
  }
...
// A non-Subject player submits their guess for the current card.
async function submitGuess({ code, playerId, guess }) {
  ...
  if (!findPairByPlayerId(playerId, room.pairs)) return { ok: false, error: 'not_in_game' };
...
async function submitTruth({ code, playerId, truth }) {
  ...
  // Block until everyone else has guessed
  var guessers = (room.players || []).filter(function (p) { return p.id !== room.currentSubjectId; });
  var missing = guessers.filter(function (p) { return !(room.guesses && room.guesses[p.id]); });
  if (missing.length > 0) {
    return { ok: false, error: 'waiting_for_guessers', waitingOn: missing.map(function (p) { return p.name; }) };
  }

  room.subjectAnswer = truth;
  var detail = scoreCard(card, room.guesses || {}, truth, room.currentSubjectId, room.pairs);
...
async function nextCard({ code, playerId }) {
  ...
  // Advance the turn pointer (and round, when we wrap)
  var nextTurn = (room.turn || 0) + 1;
  if (nextTurn >= (room.turnOrder || []).length) {
    nextTurn = 0;
    room.round = (room.round || 1) + 1;
    // Reshuffle order so the rotation doesn't get predictable across rounds
    room.turnOrder = shuffle(room.turnOrder || []);
  }
  room.turn = nextTurn;
  room.currentSubjectId = room.turnOrder[nextTurn];
