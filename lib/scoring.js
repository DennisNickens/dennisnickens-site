...
// (Pillar 1: a/b/c/d = D/I/S/C, Pillar 3: a/b/c/d = FactFinder/FollowThru/QuickStart/Implementor,
//  etc.).

const { QUESTION_MAP, PERSONALITY_QUESTION_INFO, FAITH_ORIENTATION_LABELS } = require('./question-map');

const PILLAR1_TAGS = ['D', 'I', 'S', 'C'];
const PILLAR3_TAGS = ['FactFinder', 'FollowThru', 'QuickStart', 'Implementor'];
const PILLAR4_TAGS = ['Words', 'Time', 'Touch', 'Service', 'Gifts'];
const PILLAR5_TAGS = ['Visual', 'Auditory', 'Reading', 'Doing'];
...
function buildRawAnswersFromCustomFields(customFields) {
  ...
  for (const f of customFields) {
    ...
    if (n >= 1 && n <= 24) {
      ...
      if (tag) rawAnswers.connectionLanguage.push(tag);
    } else if (n >= 93 && n <= 108) {
