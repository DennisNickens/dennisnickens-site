...
// The Subject finishes the EXPLAIN step on a Choice+Explain card by tapping
// "Done. Next card." This validates the caller is the current Subject and the
// current card actually requires an explain step, then advances exactly like
// friend-next-card (rotate Subject, advance deck, reset round state). No timer,
// no scoring; the advance is fully Subject-controlled.
async function finishExplain({ code, playerId, cardId }) {
  ...
  if (playerId !== room.currentSubjectId) return { ok: false, error: 'not_subject' };
  var card = CARDS_BY_ID[room.currentCardId];
  if (!card || !card.requireExplain) return { ok: false, error: 'not_explain_card' };
