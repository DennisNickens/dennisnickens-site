// /api/lq-sample-lead.js
// Captures a Lovers Quest sample lead from the public sample page.
// Creates (or upserts) the contact in GHL and applies the
// "Lovers Quest Sample Lead" tag so Dennis can later trigger a workflow
// to send the buy link when the full deck launches.
//
...
const LEAD_TAG = 'Lovers Quest Sample Lead';
...
export default async function handler(req, res) {
  ...
  if (!token) {
    console.error('[lq-sample-lead] Missing GHL_PRIVATE_INTEGRATION_TOKEN');
  ...
  try {
    ...
    if (!upsertResp.ok) {
      console.error('[lq-sample-lead] GHL upsert failed:', upsertResp.status, JSON.stringify(upsertData));
    ...
    if (!contactId) {
      console.error('[lq-sample-lead] No contact id in upsert response');
      return res.status(502).json({ success: false, error: 'GHL did not return a contact id' });
    }

    // Apply the "Lovers Quest Sample Lead" tag so Dennis can trigger a
    // workflow on it later (buy-link blast when full deck launches).
    let tagged = true;
    try {
      ...
      if (!tagResp.ok) {
        ...
        console.warn('[lq-sample-lead] tag add failed (non-fatal):', tagResp.status, t.slice(0, 200));
        tagged = false;
      }
    } catch (e) {
      console.warn('[lq-sample-lead] tag add threw (non-fatal):', e && e.message);
      tagged = false;
    }

    return res.status(200).json({ success: true, contact_id: contactId, tagged });
  } catch (err) {
    console.error('[lq-sample-lead] Error:', err);
