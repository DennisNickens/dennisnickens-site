...
//   Pick-one (current): { core_q1: 'a', currency_q1: 'c', ... }
//     the single option key the respondent picked.
//   Ranked (legacy):    { core_q1: {a:1,b:3,c:2,d:4}, ... }
//     rank 1 = most like the respondent. Kept so stored response
//     sets from before the pick-one UX still re-score via
//     /api/sr-score-session.
//
// Points, pick-one: the picked option's type scores 1, everything
// else scores 0. Primary type per pillar = highest pick count,
// secondary = next highest.
// Points, ranked (legacy): rank r among n options scores (n - r + 1).
//
...
function scorePillar(pillar, pillarAnswers, questionTypeMaps) {
  ...
  // Counts how often each OPTION KEY (a/b/c/d) was the top choice, for
  // position-bias (straight-lining) detection. Under pick-one this is
  // simply the pick; under legacy ranking it is the rank-1 option.
  const topChoiceCounts = {};
  let answered = 0;

  for (const [qid, answer] of Object.entries(pillarAnswers)) {
    const typeMap = questionTypeMaps[qid];
    if (!typeMap) continue;

    // Pick-one answer: a single option key string. 1 point to that
    // option's type, 0 to everything else.
    if (typeof answer === 'string') {
      const type = typeMap[answer];
      if (!type || !(type in raw)) continue;
      answered++;
      raw[type] += 1;
      topChoiceCounts[answer] = (topChoiceCounts[answer] || 0) + 1;
      continue;
    }

    // Legacy ranked answer: {a:1,b:3,c:2,d:4}. Rank r of n scores n-r+1.
    ...
    for (const [opt, rank] of Object.entries(ranks)) {
      ...
      if (rank === 1) topChoiceCounts[opt] = (topChoiceCounts[opt] || 0) + 1;
    }
  }
  ...
  if (answered >= 5) {
    const maxTops = Math.max(0, ...Object.values(topChoiceCounts));
    const ratio = maxTops / answered;
    if (ratio >= 0.9) {
      const dominantOption = Object.entries(topChoiceCounts).sort((a, b) => b[1] - a[1])[0][0];
...
function scoreSession(tier, answers, bank) {
  ...
  for (const [qid, answer] of Object.entries(answers)) {
    ...
    if (byPillar[pillar]) byPillar[pillar][qid] = answer;
  }
