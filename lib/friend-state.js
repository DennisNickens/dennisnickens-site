...
// Host taps "Start Another Game" on the game-over screen (#82). Each game is
// one-shot: instead of restarting the same room (the old Rematch, now removed),
// we DISSOLVE the finished room so any joiner still polling it disconnects, then
// mint a brand new room with a fresh code + QR. The host carries their name and
// gender into the new room. Previous joiners do NOT carry over; whoever is around
// re-scans the new code to join. The license gate is client-side, so no license
// is re-checked here.
async function startAnotherGame({ code, hostId }) {
  var room = await getRoom(code);
  if (!room) return { ok: false, error: 'room_not_found' };
  if (room.hostId !== hostId) return { ok: false, error: 'not_host' };

  var hostPlayer = (room.players || []).find(function (p) { return p.id === hostId; });
  var created = await createRoom({
    hostName: hostPlayer ? hostPlayer.name : 'Host',
    gender: (hostPlayer && hostPlayer.gender) || 'male'
  });
  if (!created || !created.room) return { ok: false, error: 'could_not_create_room' };

  // Dissolve the finished room. Joiners polling it get phase 'dissolved' and
  // eject themselves; the record expires on its normal TTL.
  room.phase = 'dissolved';
  room.dissolvedAt = new Date().toISOString();
  await saveRoom(room);

  return { ok: true, room: created.room, hostId: created.hostId, code: created.room.code };
}

// A joiner leaves at game-over (#82): remove them from the room roster (and any
// pair) so the disconnect is enforced server-side, not just hidden client-side.
// Safe to call on an already-gone room.
async function leaveGame({ code, playerId }) {
  var room = await getRoom(code);
  if (!room) return { ok: true };
  room.players = (room.players || []).filter(function (p) { return p.id !== playerId; });
  room.pairs = (room.pairs || []).filter(function (pr) { return (pr.playerIds || []).indexOf(playerId) === -1; });
  await saveRoom(room);
  return { ok: true };
}
...
// Explain is depth-aware. #86 made it universal on every scoring card; #88
// narrows it to Real + Deep only, because Light is the warm/easy tier and most
// Light cards have no meaningful "why" to explain. The gate is client-side: on
// Real/Deep the reveal shows the inline Explain input and advances through this
// endpoint; on Light the reveal hides Explain and advances through next-card.
// This endpoint stays a permissive passthrough to nextCard for ALL depths (so a
// Light client that still calls it just advances straight to the next card, no
// Explain sub-phase exists to enter), which keeps Real/Deep behavior identical
// to #86 and avoids breaking any device on a stale app shell. Talk cards have
// no reveal/answer to explain and advance via friend-next-card.
async function finishExplain({ code, playerId, cardId }) {
  ...
  // On Light the Explain step is skipped client-side; whichever advance endpoint
  // is called, the downstream behavior is identical to friend-next-card.
