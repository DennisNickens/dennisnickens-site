...
- For the CORE breakdown, do NOT emit a table. Instead, emit ONE line containing this exact marker format (the renderer replaces it with a visual horizontal bar chart that shows each letter, the style name, the bar visualization, and the percentage value, so the table would be redundant): `<!-- SCORE_BAR pillar="CORE" labels="Commander|Organizer|Relator|Energizer" values="13.0|25.0|34.0|28.0" letters="C|O|R|E" -->` Use the translation rule from earlier in this prompt to map internal D->C, I->E, S->R, C->O. Compute each customer's actual percentage to one decimal place. **MANDATORY MATH VERIFICATION BEFORE EMITTING:** the four percentages MUST sum to exactly 100.0. Before you write the marker line, add C + O + R + E in your head. If the sum is 99.9 or 100.1 (a normal rounding artifact), adjust the largest value up or down by 0.1 so the sum is exactly 100.0. If the sum is anything else (e.g. 108.0, 92.0), you have made an arithmetic error: recompute each percentage from the raw scores using (rawLetter / sumOfAllFour) * 100, then verify the sum is 100.0 again. Do NOT emit the marker line until the four values sum to exactly 100.0. Replace the values with those four verified percentages in the order C, O, R, E. Keep the pipe (|) separators exactly as shown. The marker is an HTML comment, so it does NOT render as visible text; the renderer replaces it with the bar chart. After the marker, you may include one short italic caption line if helpful (e.g. *"Percentages reflect the proportion each style contributes to your total behavioral profile."*) but do NOT repeat the data in a table.
...
- Each letter and how it shows up (Charge, Trust, Decide, Live). When you present this as a table with columns Dimension / Your Leaning / What It Means, format the "Your Leaning" cell with the letter on its own line ABOVE the leaning name, using this exact markdown: `**W**<br>Inward` (bold letter, then HTML break, then leaning name on the next line). Do NOT use the comma format "W, Inward". The renderer applies CSS that centers these cells and styles the letter as a small gold heading above the leaning word.
- Strengths
- Blind spots
...
- For the channel percentages, do NOT emit a table. Instead, emit ONE line containing this exact marker (the renderer replaces it with a visual horizontal bar chart that already shows the channel name, the bar, and the percentage value, so the table would be redundant): `<!-- SCORE_BAR pillar="CHANNEL" labels="Sight|Sound|Word|Touch" values="25.0|18.0|22.0|35.0" letters="S|N|W|T" -->` Replace the values with the customer's actual percentages to one decimal place. **MANDATORY MATH VERIFICATION BEFORE EMITTING:** the four percentages MUST sum to exactly 100.0. Add Sight + Sound + Word + Touch in your head before you write the marker. If the sum is 99.9 or 100.1 (rounding artifact), adjust the largest value by 0.1 so the sum is exactly 100.0. If the sum is anything else (e.g. 108.0, 92.0), recompute each percentage from the raw scores using (rawChannel / sumOfAllFour) * 100 and verify again. Do NOT emit the marker line until the four values sum to exactly 100.0. Keep the four values in the order Sight, Sound, Word, Touch. Keep the pipe (|) separators exactly as shown.
