...
function findOptionIndex(value, options) {
  ...
  if (idx >= 0) return idx;
  // Bare letter answer (A/B/C/D...) -> positional index. Some conditional sets
  // (Q-H Manifestation Gifts, Q-I Fruits) store a lone letter rather than the full
  // option text, which neither the exact match above nor the "X) " strip below catch.
  const m = v.match(/^([A-Fa-f])$/);
  if (m) {
